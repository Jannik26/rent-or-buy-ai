...
import { isSubscriptionRequiredMessage } from "@/lib/billing/require-active-subscription";
  ...
  const mut = useMutation({
    ...
    onError: (e: unknown) => {
      ...
      if (isSubscriptionRequiredMessage(message)) {
        ...
        toast.error(message ?? "Fehler bei der KI-Analyse");
      }
    },
  });
      ...
      <section className="rounded-2xl border border-border bg-card shadow-soft">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="font-display text-lg text-foreground tabular-nums">
                {lead.score_numeric}
                <span className="text-xs font-sans text-muted-foreground">/100</span>
              </span>
            </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <p className="max-w-[65ch] text-sm leading-relaxed text-foreground">
            {lead.ai_summary ??
              "Noch keine KI-Zusammenfassung — klicken Sie auf »KI-Analyse aktualisieren«, sobald das Gespräch beendet ist."}
          </p>
          ...
          <dl className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-3 border-t border-border/60 pt-4">
            ...
            {showAsking && (
              <Fact icon={Banknote} label="Preisvorstellung" value={lead.asking_price} muted />
            )}
    ...
    <div className="min-w-0">
      <dt
        className={cn(
          ...
          muted ? "text-[10px] text-muted-foreground/70" : "text-[11px] text-muted-foreground",
        )}
      >
      ...
      <dd
        className={cn(
          ...
          muted ? "text-xs font-medium" : "text-sm font-medium",
        )}
      >
        {value || "—"}
      </dd>
