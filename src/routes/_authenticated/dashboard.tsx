...
function Dashboard() {
  ...
  // Computed once per page load and threaded down explicitly, rather than
  // read again per component — deterministic, and avoids any drift between
  // the banner and the Embed tab within the same render.
  ...
  const companyQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from("companies")
        .select(
          "id, name, greeting, subscription_status, demo_expires_at, subscription_expires_at, plan",
        )
        .eq("owner_id", user.id)
        .maybeSingle();
  ...
  const stats = useMemo(() => {
    ...
    const qualified = leads.filter(
      (l) => l.status === "qualifiziert" || l.status === "termin" || l.score_numeric >= 60,
    ).length;
    return {
      total: leads.length,
    ...
    <>
      {tab === "leads" && (
        <div className="p-4 sm:p-8 max-w-[1600px] mx-auto w-full">
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-end gap-4 sm:flex sm:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {stats.newWeek > 0
                  ? `${stats.newWeek} neue Leads in den letzten 7 Tagen.`
                  : "Noch keine neuen Leads diese Woche."}
              </p>
            </div>
          ...
          {company && <SubscriptionBanner company={company} now={now} />}
      ...
      {tab === "embed" && company && <EmbedTab company={company} now={now} />}
      {tab === "settings" && company && (
        <SettingsTab company={company} onSaved={() => companyQuery.refetch()} />
      )}
    </>
  );
}
...
function StatCard({
  ...
  deltaLabel?: string;
  icon: typeof Flame;
  tone: "primary" | "hot" | "gold" | "success";
  hint?: string;
  progress?: number;
  to: "/leads" | "/appointments" | "/analytics";
      ...
      <div className="mt-4 min-h-[20px]">
        {progress !== undefined ? (
          <div className="h-1.5 rounded-full bg-muted overflow-hidden">
            <div
              className={cn("h-full transition-all", progressClass)}
...
function EmptyState({ onAction }: { onAction: () => void }) {
  return (
    <div className="p-12 sm:p-16 text-center">
      ...
      <button
        onClick={onAction}
        className="mt-5 inline-flex items-center gap-1.5 text-sm font-medium text-primary hover:text-secondary"
      >
  ...
  return (
    ...
    >
      ...
      <td className="px-4 py-4 text-muted-foreground">
        ...
        {l.location && (
          <div className="text-xs text-muted-foreground/70 truncate">{l.location}</div>
        )}
      </td>
      <td className="px-4 py-4 text-muted-foreground">
      ...
      <td className="px-4 py-4">
        <StatusBadge status={l.status} score={l.score} />
      </td>
...
function EmbedTab({ company, now }: { company: Company; now: Date }) {
  const [copied, setCopied] = useState(false);
  const [upgradeOpen, setUpgradeOpen] = useState(false);
  const { state } = getSubscriptionState(company, now);
  const blocked = !canAccessGatedFeatures(state);

  const base = typeof window !== "undefined" ? window.location.origin : "";
  const snippet = `<script src="${base}/embed.js" data-estateai="${company.id}" defer></script>`;
  function copy() {
    ...
    return (
      <div className="p-4 sm:p-8 max-w-3xl mx-auto">
        ...
        <div className="mt-6 rounded-2xl border border-destructive/30 bg-destructive/10 p-6 text-destructive">
          <h3 className="font-display text-lg">Widget gesperrt</h3>
          <p className="mt-1 text-sm">
            Ihr Zugang ist derzeit nicht aktiv. Aktivieren Sie ein Abo, um das Widget weiter auf
            Ihrer Website einzubetten.
          </p>
          <button
            onClick={() => setUpgradeOpen(true)}
            className="mt-4 rounded-lg bg-primary text-primary-foreground px-4 py-2 text-sm font-medium hover:bg-secondary transition"
          >
            Jetzt upgraden
          </button>
        </div>
        <UpgradeDialog open={upgradeOpen} onOpenChange={setUpgradeOpen} reason="feature_locked" />
      </div>
    );
  }
  ...
  return (
    <div className="p-4 sm:p-8 max-w-3xl mx-auto">
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        Fügen Sie diesen Code-Schnipsel vor <code>{"</body>"}</code> in Ihre Website ein. Das
        EstateAI-Widget erscheint automatisch unten rechts.
      </p>
      <div className="mt-6 rounded-2xl bg-primary text-primary-foreground p-5 font-mono text-xs sm:text-sm relative overflow-x-auto shadow-elegant">
        ...
        <button
          onClick={copy}
          className="absolute top-3 right-3 inline-flex items-center gap-1.5 rounded-lg bg-gold text-gold-foreground px-3 py-1.5 text-xs font-medium hover:opacity-90"
        >
      ...
      <div className="mt-8 grid sm:grid-cols-3 gap-4">
        {[
          ...
          <div key={s.i} className="rounded-2xl border border-border bg-card p-5 shadow-soft">
            <div className="size-8 rounded-lg bg-gradient-navy text-gold grid place-items-center font-display">
              {s.i}
            </div>
      ...
      <div className="mt-8 rounded-2xl border border-border bg-card p-5 shadow-soft">
        <h3 className="font-display text-lg flex items-center gap-2">
          <Building2 className="size-4 text-gold" /> Vorschau
        </h3>
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-4 inline-flex items-center gap-1.5 text-sm font-medium hover:text-gold"
        >
      ...
      >
        ...
        <div>
          ...
          <p className="mt-1.5 text-xs text-muted-foreground">
            Die erste Nachricht, die Besucher sehen.
          </p>
        </div>
        <button
          type="submit"
